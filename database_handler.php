...
class DotacniKalkulatorDB
{
    ...
    private function ulozitHlavniZadost($uuid, $form_data, $ip_address, $user_agent)
    {
        ...
        $stare_uuid = null;
        if (isset($form_data["current_uuid"])) {
            $zadost = $this->nacistZadost($form_data["current_uuid"]);
            if ($zadost["uuid"]) {
                $stare_uuid = $zadost["uuid"];
            }
        }

        $sql = "INSERT INTO dotacni_kalkulator_zadosti 
                (uuid, typ_nemovitosti, rok_vystavby, ip_adresa, user_agent, utm_zdroj, uuid_source) 
                VALUES (?, ?, ?, ?, ?, ?, ?)";
